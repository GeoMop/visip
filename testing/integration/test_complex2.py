...
def gmsh_run(self, geometry: wf.FileIn, mesh_step: float = 1.0) -> wf.FileIn:
    ...
    self.mesh_res = wf.system([os.path.join(script_dir, 'gmsh.sh'), geometry, "-2", "-clscale",
...
def simple_wf(self, mesh_step) -> wf.ExecResult:
    ...
    self.res = wf.system([os.path.join(script_dir, 'flow.sh'), self.flow_input, "-o", "."],
                         stdout=wf.SysFile.PIPE, stderr=wf.SysFile.STDOUT,
                         input_files = ["darcy_flow.yaml", "square.msh"],
                         output_files = ["flow_fields/flow_fields-000000.vtu", "flow_fields.pvd"])
    return self.res
