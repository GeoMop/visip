...
def gmsh_run(self, geometry: wf.FileIn, mesh_step: wf.Float) -> wf.FileIn:
    ...
    Value_5 = '{:8.2g}'
    A_list_1 = ['../gmsh.sh', geometry, '-2', '-clscale', wf.format(Value_5, mesh_step), '-format', 'msh2', '-o', self.mesh_file_out]
    Value_9 = '.msh_log'
    self.mesh_res = wf.system(arguments=A_list_1, stdout=wf.derived_file(f=geometry, ext=Value_9), stderr=wf.SysFile.STDOUT, workdir='')
...
def simple_wf(self, mesh_step) -> wf.ExecResult:
    ...
    A_list_1 = ['../flow.sh', self.flow_input, '-o', '.']
    self.res = wf.system(arguments=A_list_1, stdout=wf.SysFile.PIPE, stderr=wf.SysFile.STDOUT, workdir='')
    return self.res