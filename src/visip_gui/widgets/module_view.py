...
from visip_gui.menu.module_view_menu import ModuleViewMenu
            ...
            if issubclass(type(wf), _Workflow):
                ...
                if wf.slots:
                    for slot in wf.slots:
                        ...
                        input_item.setIcon(0, QIcon(os.path.join(os.getcwd(), "visip_gui\\icons\\arrow_right.png")))

                if wf._result_call:
                    output_item = QTreeWidgetItem(item, [wf.result.name])
                    output_item.setIcon(0, QIcon(os.path.join(os.getcwd(), "visip_gui\\icons\\arrow_left.png")))
